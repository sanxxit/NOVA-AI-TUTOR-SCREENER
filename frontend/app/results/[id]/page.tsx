...
export default function ResultsPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      label('Reference ID:', data.candidate_id);
      label('Date:', new Date(data.completed_at).toLocaleDateString('en-US', {
      ...
      const verdictRGB: [number, number, number] = data.passed ? [16, 185, 129] : [239, 68, 68];
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(22);
      doc.setTextColor(...verdictRGB);
      doc.text(data.passed ? 'PASS' : 'FAIL', pageW / 2, y, { align: 'center' });

      y += 7;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.setTextColor(100, 100, 100);
      doc.text(`Overall Score: ${data.overall_score.toFixed(1)} / 5.0`, pageW / 2, y, { align: 'center' });
      ...
      autoTable(doc, {
        ...
        body: DIM_ORDER.map((key) => {
          const dim  = data.scores?.dimensions?.[key];
      ...
      doc.save(`nova-report-${data.candidate_id}.pdf`);
    };
    run().catch(() => { /* PDF generation failed silently */ });
  }, [data]);
